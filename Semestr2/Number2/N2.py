...
class GaussianDiff:
    ''' Реализуем класс с уравнением плоской волны для модулированного гауссова сигнала в дискретном виде
...
if __name__ == '__main__':
    ...
    layer_x_2=layer_x+0.05
    layer_x_2_DX=int(np.ceil(layer_x_2 / d))
    layer_x_3=layer_x+0.05+0.05 
    layer_x_3_DX=int(np.ceil(layer_x_3 / d))
    layer_x_4 = layer_x_3 + 0.05
    ...
    eps = np.ones(maxSize)
    eps[layer_x_DX:] = 8.2 
    eps[layer_x_2_DX:] = 6.7
    eps[layer_x_3_DX:] = 8.2
    eps[layer_x_4_DX:] = 3.0
    ...
    source = GaussianDiff(dt, 100, 15e9,200)
    ...
    display.drawBoundary(layer_x_4_DX)
